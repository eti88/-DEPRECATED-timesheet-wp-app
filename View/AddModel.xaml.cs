...
namespace timesheet.View
{
    ...
    /// </summary>
    public sealed partial class AddModel : Page
    {
        ...
        private async void AddModel_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (string.IsNullOrEmpty(txtModelDesc.Text))
                {
                    txtModelDesc.Text = "";
                }
                Db_Helper.InsertModel(new ActivityModel(txtModelName.Text, txtModelDesc.Text));
                NavigateToList();
            }
            else
            {
                MessageDialog msgDialog = new MessageDialog("Inserisci almeno il nome del modello!");
